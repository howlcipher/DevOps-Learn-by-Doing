...
scaled by OperatingMode and ExplanationDepth.
...
from devops_learn.domain.enums import ExplanationDepth, OperatingMode
from devops_learn.domain.explanation_models import Explanation, LearningMoment


class ExplanationService:
    def render(
        self, explanation: Explanation, *, mode: OperatingMode, depth: ExplanationDepth
    ) -> str:
        if mode is OperatingMode.AUTOPILOT and depth < ExplanationDepth.DEEP:
    ...
    def render_learning_moment(
        self, moment: LearningMoment, *, mode: OperatingMode, depth: ExplanationDepth
    ) -> str | None:
        if mode is OperatingMode.AUTOPILOT and depth < ExplanationDepth.DEEP:
            return None
